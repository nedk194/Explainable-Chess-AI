...
// pawn struc method
Bitboard PAWN_FRONT_SPAN[NCOLORS][NSQUARES];
int passedRank[8] = { 0, 5, 10, 20, 35, 60, 100, 0 };
...
void initialise_pawn_front_span() {
    for (int i = 0; i < 64; i++) {
        ...
        PAWN_FRONT_SPAN[WHITE][i] = frontSpan << (8 * (rank + 1));
        PAWN_FRONT_SPAN[BLACK][i] = frontSpan & ~(0xFFFFFFFFFFFFFFFF << (8 * rank));
	}
}
// maps a single square to a bitboard with that square only
Bitboard BB_SQUARE[64];

void initialise_bb_squares() {
    for (int i = 0; i < 64; ++i) {
        BB_SQUARE[i] = 1ULL << i;
    }
}
